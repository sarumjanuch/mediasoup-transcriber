...
import { CapabilitiesRequest, ConsumerClosedNotification, CreateProducerRequest, PauseProducerRequest, ResumeProducerRequest, RtpCapabilitiesNotification, TransportConnectedNotification, TransportInfo, TransportInfoRequest } from "./MessageTypes";
...
export class Server {
    ...
    private async _initClient(ws: WebSocket, call: Call, userId: string, clientId?: string): Promise<void> {
        ...
        const transcriptionListener = (transcription: Transcription) => {
            if (!comlink) return;
            // logger.info("Transcription send", transcription);
            comlink!.sendTranscriptionReadyNotification(transcription);
        };
        call.onTranscriptionEmitted(transcriptionListener);
        comlink = await Comlink.builder()
            ...
            .onClosed(() => {
                call.offTranscriptionEmitted(transcriptionListener);
